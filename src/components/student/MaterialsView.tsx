...
import { DRIVE_MATERIAL_URL, DRIVE_PAPERS_URL } from "./DashboardGrid";

interface MaterialsViewProps {
  onBack: () => void;
  initialTab?: "all" | "material" | "paper";
}

export default function MaterialsView({ onBack, initialTab = "all" }: MaterialsViewProps) {
  const [items, setItems] = useState<StudyMaterial[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<"all" | "material" | "paper">(initialTab);
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      <div className="flex justify-between items-center px-1">
        <div>
          <h2 className="text-white font-extrabold text-lg tracking-tight">MCA Resources</h2>
          <p className="text-neutral-500 text-xs font-bold mt-0.5">Study notes, lecture slides & old exams</p>
        </div>
        <div className="bg-cyan-950/40 border border-cyan-800/50 px-3 py-1 rounded-lg text-xs font-bold text-cyan-400">
          {isLoading ? "..." : `${filteredItems.length} Available`}
        </div>
      </div>

      {/* Quick Google Drive Master Folder Shortcuts */}
      <div className="grid grid-cols-2 gap-2">
        <a
          href={DRIVE_MATERIAL_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="p-3 rounded-xl bg-gradient-to-r from-blue-950/40 to-neutral-900 border border-blue-800/40 hover:border-blue-500/60 transition-all flex items-center justify-between group"
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-bold text-white group-hover:text-blue-300 transition-colors">Master Drive</p>
              <p className="text-[10px] text-neutral-400">All Study Notes</p>
            </div>
          </div>
          <svg className="w-3.5 h-3.5 text-neutral-500 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="p-3 rounded-xl bg-gradient-to-r from-purple-950/40 to-neutral-900 border border-purple-800/40 hover:border-purple-500/60 transition-all flex items-center justify-between group"
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-bold text-white group-hover:text-purple-300 transition-colors">Papers Drive</p>
              <p className="text-[10px] text-neutral-400">Exam Question Papers</p>
            </div>
          </div>
          <svg className="w-3.5 h-3.5 text-neutral-500 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
      ...
      {/* Main Tabs (All / Study Material / Old Papers) */}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by topic, unit, exam year, or faculty..."
          className="w-full pl-9 pr-3 py-3 rounded-xl bg-neutral-900 border border-neutral-800 text-white text-xs placeholder-neutral-500 outline-none focus:border-cyan-500/60 transition-all font-medium shadow-inner"
        />
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center p-12 space-y-3">
          ...
          <p className="text-xs font-bold text-neutral-500">Loading resources...</p>
        </div>
        ...
        <div className="p-8 text-center rounded-2xl bg-neutral-900/60 border border-neutral-800/80 space-y-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-neutral-200">No Resources Found</h3>
            <p className="text-xs text-neutral-500 mt-1 max-w-xs mx-auto">
              {searchQuery || selectedSubject !== "ALL" || activeTab !== "all"
                ? "No materials or papers match your filter criteria. Try clearing search or filters."
                : "No files uploaded yet. You can still use the Master Google Drive buttons above."}
            </p>
          </div>
        </div>
      ) : (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  >
                    {item.type === "material" ? "Study Material" : "Question Paper"}
                  </span>
              ...
              <div className="flex justify-between items-center mt-3 pt-3 border-t border-neutral-800/60 text-xs">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="font-bold text-cyan-400 hover:text-cyan-300 flex items-center gap-1.5 bg-cyan-950/40 border border-cyan-800/50 px-3 py-1.5 rounded-xl hover:scale-105 transition-all text-xs"
                >

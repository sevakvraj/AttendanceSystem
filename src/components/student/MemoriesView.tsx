...
// Initial photos strictly ordered by ID sequence 1, 2, 3, 4, 5, 6
const INITIAL_FRESHERS_PHOTOS: PhotoItem[] = [
  {
    id: 1,
    order: 1,
    displayId: "01",
...
export default function MemoriesView({ onBack }: MemoriesViewProps) {
  const [activeCategory, setActiveCategory] = useState<string>("all");
  const [photos, setPhotos] = useState<PhotoItem[]>(INITIAL_FRESHERS_PHOTOS);
    ...
    fetch("/api/memories")
      ...
      .then((data) => {
        if (data?.photos && Array.isArray(data.photos) && data.photos.length > 0) {
          const sorted = [...data.photos].sort((a: any, b: any) => (Number(a.id) || 0) - (Number(b.id) || 0));
          setPhotos(sorted);
        }
      })
      .catch(() => {});
  ...
  // Split into 2 balanced columns for Pinterest masonry view
  const col1 = filteredPhotos.filter((_, i) => i % 2 === 0);
  const col2 = filteredPhotos.filter((_, i) => i % 2 === 1);

  return (
    <div className="space-y-6 sm:space-y-8 pb-16 animate-fade-in text-white select-none">
      {/* Top Back Navigation Bar */}
      <div className="flex items-center justify-between">
        <button
          onClick={onBack}
          className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-neutral-900/80 border border-neutral-800 text-neutral-300 hover:text-white hover:bg-neutral-800 text-xs font-semibold transition-all group cursor-pointer"
        >
          <svg
        ...
        <span className="text-[10px] tracking-widest font-bold uppercase text-amber-400 bg-amber-950/40 px-3 py-1 rounded-full border border-amber-500/30">
          Vault &bull; MCA &apos;26-&apos;27
        </span>
      </div>

      {/* Cinematic Editorial Header ("The Archive") */}
      ...
      {/* Minimalist Categorical Navigation */}
      <div className="flex items-center justify-start sm:justify-center gap-5 sm:gap-7 overflow-x-auto pb-2 scrollbar-none pt-2">
        {CATEGORIES.map((tab) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center group cursor-pointer transition-colors whitespace-nowrap"
            >
              <span
      ...
      {/* Two-Column Masonry Stream (Edge-to-Edge Visual Cards) */}
      {filteredPhotos.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4.5">
          {/* Column 1 */}
          <div className="flex flex-col space-y-3 sm:space-y-4.5">
            {col1.map((photo) => {
              ...
              return (
                <div
                  ...
                  className="group relative rounded-2xl sm:rounded-3xl overflow-hidden bg-neutral-950 border border-white/5 hover:border-amber-400/80 shadow-md hover:shadow-xl hover:shadow-amber-400/15 transition-all duration-300 cursor-pointer"
                >
                  {/* Natural Aspect Ratio Image - Free Size as per photo */}
                  <img
                    src={photo.image}
                    alt={photo.title}
                    loading="lazy"
                  ...
                  {/* Dark Cinematic Hover Overlay with Gold Tag & Tap To View */}
                  <div className="absolute inset-0 bg-black/55 backdrop-blur-[1.5px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-3 text-center">
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-amber-950/70 border border-amber-400/60 text-amber-300 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider shadow-sm">
                      {photo.tag}
                    </span>
                  </div>
                </div>
      ...
      {selectedPhoto && selectedIndex !== null && (
        ...
        >
          <div
            ...
            className="max-w-3xl w-full bg-neutral-950 border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl relative flex flex-col cursor-default max-h-[94vh]"
          >
            {/* Modal Header Bar */}
            <div className="p-3.5 sm:p-4 border-b border-neutral-900 flex items-center justify-between bg-black/70">
              <div className="flex items-center space-x-2.5">
                <span className="text-[10px] font-bold uppercase tracking-wider text-amber-300 bg-amber-950/50 px-2.5 py-0.5 rounded-full border border-amber-500/40">
                  {selectedPhoto.tag}
                </span>
              ...
              <div className="flex items-center space-x-2">
                ...
                <a
                  ...
                  className="px-3 py-1 rounded-full bg-neutral-900 hover:bg-neutral-800 border border-neutral-700 text-xs font-semibold text-neutral-200 inline-flex items-center space-x-1.5 transition-colors cursor-pointer"
            ...
            {/* Centered Image with Slide Navigation Buttons */}
            <div className="relative w-full flex-1 min-h-[320px] max-h-[68vh] bg-black flex items-center justify-center overflow-hidden">
              <img

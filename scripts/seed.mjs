...
async function seed() {
  console.log(`Starting to seed ${STUDENTS.length} students into Firestore...`);
  let count = 0;
  for (const s of STUDENTS) {
    const ref = doc(db, "students", s.pid);
    await setDoc(ref, {
      name: s.name,
      password: s.pid, // Password same as PID
      phone: s.phone,
      email: s.email,
      rollNo: s.rollNo,
    ...
    count++;
    console.log(`[${count}/${STUDENTS.length}] Added: ${s.pid} - ${s.name}`);
  }
  console.log(`\n🎉 Successfully added all ${count} students!`);
  process.exit(0);
}
